...
int main()
{
    ...
    printf("Please Enter Number to Search in Linked List \n");
    scanf("%d",&No);

    iRet = SearchFirstOccurance(Head,No);
    if(iRet == -1)
    {
    ...
    iRet = SearchLastOccurance(Head,No);
    if(iRet == -1)
    {
        printf("There is no such element in LL\n");
    }
    ...
    {
        printf("element is Last Occurance  : %d\n",iRet);
    }
